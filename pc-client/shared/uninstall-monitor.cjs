...
async function waitForUninstallation({
  check,
  wait = delay,
  intervalMs = 2500,
  maxAttempts = 240,
  ...
  if (typeof check !== "function" || typeof wait !== "function") {
    throw new Error("卸载监控参数无效");
  }
  if (!Number.isSafeInteger(maxAttempts) || maxAttempts < 1) {
    throw new Error("卸载监控次数无效");
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (
      lastStatus &&
      lastStatus.installed === false &&
      lastStatus.detection === "absent"
    ) {
      return {
        outcome: "uninstalled",
        attempts: attempt,
...
module.exports = { waitForUninstallation };
