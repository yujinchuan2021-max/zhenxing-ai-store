...
const browserTypes = fs.readFileSync(path.join(root, "src", "vite-env.d.ts"), "utf8");
const index = fs.readFileSync(path.join(root, "index.html"), "utf8");
const packageJson = JSON.parse(fs.readFileSync(path.join(root, "package.json"), "utf8"));

test("desktop chrome uses compact neutral app controls instead of the native red close zone", () => {
  ...
  assert.match(windowOptions, /frame:\s*false/);
  assert.doesNotMatch(windowOptions, /titleBarStyle|titleBarOverlay/);
  ...
  for (const action of ["minimize", "toggle-maximize", "close"]) {
    assert.match(main, new RegExp(`ipcMain\\.handle\\("window:${action}"`));
    assert.match(preload, new RegExp(`ipcRenderer\\.invoke\\("window:${action}"\\)`));
  }
  assert.match(main, /BrowserWindow\.fromWebContents\(event\.sender\)/);
  assert.match(browserTypes, /minimizeWindow\(\): Promise<boolean>/);
  assert.match(browserTypes, /toggleMaximizeWindow\(\): Promise<boolean>/);
  assert.match(browserTypes, /closeWindow\(\): Promise<boolean>/);
  assert.match(chrome, /data-aihub-window-controls/);
  assert.match(chrome, /window\.aihubPC\?\.minimizeWindow\(\)/);
  assert.match(chrome, /window\.aihubPC\?\.toggleMaximizeWindow\(\)/);
  assert.match(chrome, /window\.aihubPC\?\.closeWindow\(\)/);
  assert.match(styles, /\.windowControl\s*\{[\s\S]*?width:\s*34px[\s\S]*?height:\s*30px/);
  assert.match(styles, /\.windowControl:hover\s*\{[\s\S]*?background:\s*color-mix/);
  const controlsStyles = styles.slice(
    styles.indexOf(".windowControls {"),
    styles.indexOf(".clientUpdateBadge {")
  );
  assert.doesNotMatch(controlsStyles, /danger|#(?:f00|ff0000|e81123)|\bred\b/i);
  assert.doesNotMatch(styles, /--window-controls-width/);
  assert.match(
    styles,
...
test("brand easter egg only closes from its explicit acknowledgement button", () => {
  const modal = app.slice(
    app.indexOf("opened={brandEasterEggOpen}"),
    app.indexOf("</Modal>", app.indexOf("opened={brandEasterEggOpen}"))
  );

  assert.ok(modal.length > 0, "brand easter egg modal must remain observable");
  assert.match(modal, /onClose=\{\(\) => undefined\}/);
  assert.match(modal, /closeOnClickOutside=\{false\}/);
  assert.match(modal, /closeOnEscape=\{false\}/);
  assert.match(
    modal,
...
test("installed management removes redundant overlines and the shell uses one accent hue", () => {
  ...
  assert.doesNotMatch(installedPage, /uiText\("extensions\.managementEmpty"\)/);
