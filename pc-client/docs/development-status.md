...
- Windows 安装、升级、用户数据保留和卸载的隔离验收工具；当前本地候选版本为 `0.1.23`。
...
- 一键本地发布会强制重建后台、身份与社区镜像，并逐文件比对宿主源码和运行容器 SHA-256；发现旧镜像时自动无缓存重建一次，仍不一致则拒绝发布。
- 本地发布使用可回滚事务：测试、构建和依赖审计在切换前执行；切换后的 HTTPS 与打包客户端验收失败会自动恢复上一版本。
...
当前本地后台基线为 49 个厂商、148 个产品；`0.1.23` 发布源将在固定源码提交后重新生成，并使用同时绑定 Setup 与 Portable 的 schema v2 来源证明。
...
5. 490 项单元测试与 TypeScript/Vite 生产构建通过；`0.1.23` 打包客户端发布门禁要求接受远程签名目录、同版本签名更新，通过真实 DOM 完成扩展安装/卸载，并真实下载至少 1 MiB 后进入暂停状态且保留对应部分文件。
