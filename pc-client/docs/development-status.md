...
- 116 个厂商、244 个一级产品的后台签名目录；其中 22 个产品进入“AI 可接入厂商”目录，同一可视化产品的官网、Web 与 Windows 入口集中在一张卡片，CLI 保持独立。
- Skill、MCP 和插件使用顶层资源记录与独立商店，按“资源类型 → 目标厂商 → 目标产品”展示；产品页不再维护扩展子目录。
- 43 个顶层生态资源及 125 条目标关系；第一批官方 AI 接入扩充覆盖 Figma、Notion、Atlassian、GitHub、Docker、Cloudflare、Linear、Sentry、Stripe、Supabase、Vercel、Slack、JetBrains、Canva、Postman、Asana、HubSpot 和 Chrome DevTools。尚无固定客户端配置的资源只打开官方说明，不显示一键安装。
...
当前目录基线共 116 个厂商、244 个一级产品、43 个顶层生态资源；schema v2 在不复制厂商或资源记录的前提下提供双目录视图和三个资源商店。普通 Windows 图形产品先使用厂商官方下载模块，完成逐产品生命周期审计后才能升级为受管安装。`0.1.27` 发布源将在固定源码提交后重新生成，并使用同时绑定 Setup 与 Portable 的来源证明。
