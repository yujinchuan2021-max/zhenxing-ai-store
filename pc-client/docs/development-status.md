...
- Windows 安装、升级、用户数据保留和卸载的隔离验收工具；当前本地候选版本为 `0.1.25`。
...
当前本地后台基线为 49 个厂商、148 个产品；`0.1.25` 发布源将在固定源码提交后重新生成，并使用同时绑定 Setup 与 Portable 的 schema v2 来源证明。
...
5. 635 项单元测试与 TypeScript/Vite 生产构建通过；`0.1.25` 打包客户端发布门禁要求接受远程签名目录、同版本签名更新，通过真实 DOM 完成扩展安装/卸载，并真实下载至少 1 MiB 后进入暂停状态且保留对应部分文件。
