...
- Windows 安装、升级、用户数据保留和卸载的隔离验收工具；当前本地候选版本为 `0.1.22`。
- 安装包构建来源清单：版本、安装包字节、Git 提交及工作区状态会一起进入由更新密钥签名的来源证明；生产发布只接受干净且带同版本标签的源码。
- 隔离 Portable 发布门禁会读取远程签名目录、检查签名更新、安装/卸载扩展，并真实下载至少 1 MiB 后确认暂停成功。
...
当前本地后台基线为 49 个厂商、148 个产品；`0.1.22` 发布源将在固定源码提交后重新生成，并使用带源码来源的 schema v2。
...
5. 459 项单元测试与 TypeScript/Vite 生产构建通过；`0.1.22` 打包客户端发布门禁要求接受远程签名目录、同版本签名更新，并真实下载至少 1 MiB 后进入暂停状态。
