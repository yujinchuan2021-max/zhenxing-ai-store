...
- 375 个厂商、615 个一级产品的后台目录；同一可视化产品的官网、Web 与 Windows 入口集中在一张卡片，CLI 保持独立。
...
- 首批固定资源模块已经闭环：Codex Skill、Codex MCP 与 Claude Code 插件支持检测、安装、更新、修复和精确卸载，适用时还支持启用/停用；“已安装”页从本地收据独立列举，后台停用资源或离线时仍保留安全卸载入口。
...
- Codex CLI、Claude Code、Gemini CLI 与 OpenClaw CLI 的固定 npm 模块支持隔离安装、更新、修复、命令窗口启动与收据所有权卸载；更新/修复失败会恢复原版本和原收据。
...
- Windows 安装、升级、用户数据保留和卸载的隔离验收工具；本轮本地候选版本为 `0.1.29`。
...
当前目录基线共 375 个厂商、615 个一级产品、146 个顶层生态资源；schema v2 在不复制厂商或资源记录的前提下提供双目录视图和四个资源商店。204 个厂商使用逐项核验并落库的官方图标，另有 171 个厂商使用审核文字兜底。本地后台已校验并签名发布目录 v61，草稿修订为 64，后台保持 ready；四个固定 npm CLI 已发布更新/修复能力，首批三个资源已发布受管安装 profile。普通 Windows 图形产品先使用厂商官方下载模块，完成逐产品生命周期审计后才能升级为受管安装。
...
- 对 Codex CLI、Claude Code、Gemini CLI、OpenClaw CLI 及首批 Skill/MCP/插件完成真实网络、权限、宿主版本、更新、修复和卸载验收。
...
5. 0.1.29 发布前全量单元测试与 TypeScript 检查通过；打包客户端发布门禁要求接受远程签名目录与签名更新，通过真实 DOM 验证宿主缺失时不提供扩展安装动作，并真实下载至少 1 MiB 后进入暂停状态且保留对应部分文件。真实 Codex/Claude 宿主下的扩展安装与卸载仍由人工验收。
