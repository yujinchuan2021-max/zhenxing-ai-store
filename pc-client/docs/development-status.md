...
- Windows 安装、升级、用户数据保留和卸载的隔离验收工具；本轮本地候选版本为 `0.1.28`。
...
5. 0.1.28 发布前全量单元测试与 TypeScript 检查通过；打包客户端发布门禁要求接受远程签名目录、同版本签名更新，通过真实 DOM 完成扩展安装/卸载，并真实下载至少 1 MiB 后进入暂停状态且保留对应部分文件。
