...
- 342 vendors
- 547 top-level products
- 124 ecosystem resources
- Product modules: 200 `web`, 224 `desktop-official`, 30 `cli-official`, 26 `desktop-reviewed`, 14 managed `cli`, 1 `local-model`, 52 `tutorial`
...
- Backend draft revision: 61
- Active catalog version: 58
- Release ID: `catalog-v00000058-1bd24761aa84-4bc06d4c`
- Local backend health: `http://127.0.0.1:4173/health` → `{"status":"ok"}`
- Publish warnings are expected: client auto-update is disabled, 193 vendors use reviewed letter fallbacks, and 27 managed desktop products still await physical acceptance.
