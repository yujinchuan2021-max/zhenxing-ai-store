...
下列“缺口”是本轮开始时的审计基线；本轮已完成首批 Skill、MCP、Claude 插件与固定 npm CLI 的安装、更新、修复、启停/打开和卸载闭环。当前实现仍坚持只扩充本地审核 profile，不把目录链接伪装成可执行安装。
...
目录快照收据 v3 记录宿主/profile/Adapter、固定版本、源与目标逐文件清单、拥有路径和每次安装唯一标记；配置型 Adapter 另记录其固定键和值。卸载只删除仍与收据和实例标记同时匹配的 AI Hub 文件/键；发现用户修改时进入 `modified` 并保留数据。
...
5. 写临时同盘目录，校验后原子换入。
6. 写收据 v3 和目标目录内的唯一实例标记；安装后重新读取目标并同时比较标记与清单。
