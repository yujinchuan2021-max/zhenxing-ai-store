...
枕星AI助手是一个以厂商为第一层的 Windows AI 产品、AI 可接入产品与生态资源目录。页面内容由后台目录驱动，本机探测、下载、校验、安装和卸载能力由客户端固定白名单控制。
...
- 独立 Skill、MCP、插件和连接器商店，统一按“资源类型 → 目标工具 → 资源列表 → 单项详情”浏览；工具卡同时显示所属厂商，产品页不再显示扩展资源子目录。
